"""WSD Webapp — Wood Surface Defect Detection."""
...
app = FastAPI(title="WSD - Wood Surface Defect Detection")
app.mount(
...
async def detection_page(request: Request):
    ...
    return render(
        "detection.html", request=request, active="detection", classes=classes
    )
...
async def detect_image(file: UploadFile = File(...), conf: float = 0.4):
    ...
    if model is None:
        return JSONResponse(
            {
                "error": "YOLOv8 model not found. Train via wood_defect_yolov8_training.ipynb and save best.pt to models/"
            },
            status_code=400,
        )
...
async def detect_video(
    file: UploadFile = File(...), conf: float = 0.4, stride: int = 1
):
    model = load_yolo()
    if model is None:
        return JSONResponse(
